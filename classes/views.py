...
def reserve_class(request, class_id):
    ...
    if request.method == 'POST':
        ...
        if Booking.objects.filter(user_booking=user_profile,
                                  gym_class=gym_class).exists():
            messages.error(
                request, 'You have already reserved this class.')
            return redirect('classes_list')

        # Create a new reservation
        reservation = Booking.objects.create(user_booking=user_profile,
                                             gym_class=gym_class)
        reservation.save()
...
def edit_class(request, class_id):
    ...
    if request.method == 'POST':
        if 'save' in request.POST:
            form = GymClassForm(request.POST,
                                request.FILES, instance=gym_class)
            if form.is_valid():
                form.save()
    ...
    return render(request,
                  'edit_class.html', {'form': form, 'gym_class': gym_class})
